  ...
  Future<void> syncCurrentMonthMetrics() async {
    try {
      ...
      for (final mStr in sortedMonths) {
        ...
        double round2(num? val) => double.parse((val ?? 0.0).toStringAsFixed(2));
        ...
        payloadRows.add({
          ...
          'net_cash_position': round2(monthNetCash),
          'net_cash_flow': round2(netCashFlow),
          'monthly_income': round2(monthlyIncome),
          'monthly_expenses': round2(-monthlyExpenses),
          'monthly_transfers': round2(netTransfers),
          'total_miles_balance': round2(totalMiles),
          'total_cashback_earned': round2(totalCashback),
          'income_salary': round2(incomeCategoryMap['Salary']),
          'income_interest': round2(incomeCategoryMap['Interest']),
          'income_investments': round2(incomeCategoryMap['Investments']),
          'income_dividends': round2(incomeCategoryMap['Dividends']),
          'income_other': round2(incomeCategoryMap['Other Income']),
          'expense_groceries': round2(-(expenseCategoryMap['Groceries'] ?? 0.0)),
          'expense_transport': round2(-(expenseCategoryMap['Transport'] ?? 0.0)),
          'expense_shopping': round2(-(expenseCategoryMap['Shopping'] ?? 0.0)),
          'expense_dining': round2(-(expenseCategoryMap['Dining'] ?? 0.0)),
          'expense_education': round2(-(expenseCategoryMap['Education'] ?? 0.0)),
          'expense_utilities': round2(-(expenseCategoryMap['Utilities'] ?? 0.0)),
          'expense_investments': round2(-(expenseCategoryMap['Investments'] ?? 0.0)),
          'expense_tax': round2(-(expenseCategoryMap['Tax'] ?? 0.0)),
          'expense_healthcare': round2(-(expenseCategoryMap['Healthcare'] ?? 0.0)),
          'expense_petrol': round2(-(expenseCategoryMap['Petrol'] ?? 0.0)),
          'expense_entertainment': round2(-(expenseCategoryMap['Entertainment'] ?? 0.0)),
          'expense_other': round2(-(expenseCategoryMap['Other expenses'] ?? 0.0)),
